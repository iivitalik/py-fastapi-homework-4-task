...
ACTIVATION_LINK_TEMPLATE = "http://test.com/api/v1/accounts/activate/?token={token}"
RESET_LINK_TEMPLATE = "http://test.com/api/v1/accounts/reset-password/?token={token}"
LOGIN_LINK = "http://test.com/api/v1/accounts/login/"
LOGIN_LINK_LOCAL = "http://127.0.0.1/accounts/login/"
...
async def register_user(
    ...
    try:
        ...
        activation_link = ACTIVATION_LINK_TEMPLATE.format(token=activation_token.token)
...
async def activate_account(
    ...
    token_expires_at = (
        cast(datetime, token_record.expires_at).replace(tzinfo=timezone.utc)
        if token_record
        else None
    )

    if not token_record or (token_expires_at and token_expires_at < now_utc):
        if token_record:
...
async def request_password_reset_token(
    ...
    reset_link = RESET_LINK_TEMPLATE.format(token=reset_token.token)
    background_tasks.add_task(
